...
export const ExpiredTickets = () => {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const response = await getTicketsByStatus(PaymentStatusEnum.EXPIRED);
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Boletos Vencidos</DialogDescription>
        </DialogHeader>
        {loading && (
        ...
        {!loading && !error && (
          <ScrollArea className="h-64 pr-2">
            <ul className="space-y-4 text-sm">
              {tickets.length === 0 ? (
                <li className="text-muted-foreground">Nenhum boleto vencido</li>
              ) : (
                tickets.map((ticket) => (
                  <li
                    key={ticket.id}
